...
if (!($is_auth['auth_mod']) && ($forum_topic_data['post_approval'] != POST_APPROVED))
{
	if (!defined('STATUS_404')) define('STATUS_404', true);
	message_die(GENERAL_MESSAGE, 'NO_TOPIC');
}
...
if ($row = $db->sql_fetchrow($result))
{
	do
	{
		if($row['user_id'] > 0 && ($is_auth['auth_mod'] || $row['post_approval'] == POST_APPROVED))
		{
...
//<!-- END Unread Post Information to Database Mod -->
	...
	$template->assign_block_vars('postrow', array(
		...
		'DELETED' => $postrow[$i]['post_approval'] != POST_APPROVED,
