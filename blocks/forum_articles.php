	...
	function cms_block_forum_articles()
	{
		...
		if (!empty($single_post_id)) // single post
		{
			...
			for ($i = 0; $i < sizeof($fetchposts); $i++)
			{
				...
				$topic_link = create_server_url(true) . $index_file . $prefix . 'post_id=' . $single_post_id;
				$template->assign_block_vars('articles_fa', array(
					...
					'U_EMAIL_TOPIC' => append_sid('tellafriend.' . PHP_EXT . '?topic_title=' . urlencode(ip_utf8_decode($fetchposts[$i]['topic_title'])) . '&amp;topic_link=' . urlencode($topic_link)),
					)
				);
