	...
	*/
	addItem: function(data, odd)
	{
		...
		for(var i = 0; i < cmsSettings.data.list[data.bs_id].length; i++)
		{
			...
			used += '<li>';
			if(row.layout > 0) used += cmsEditorLang.blockUsedLayout;
			else if(row.special > 0) used += cmsEditorLang.blockUsedLayoutSpecial;
			else used += cmsEditorLang.blockUsedLayoutGlobal;
	...
	infoBox: function(boxText, boxDisplay)
	{
		if (boxDisplay)
		{
			...
			if(boxText == cmsEditorLang.savingChanges)
			{
				...
				$('#sort-info-box').fadeIn(500);
				setTimeout(function() { $('#result-box').effect('highlight', {}, 500); }, 500);
			}
			else
			{
...
var cmsEditor = {
	...
	*/
	blockHTML: function(data)
	{
		...
		for(var i = 0; i < cmsEditor.data.all.length; i++)
		{
			...
			html += '>' + cmsEditor.data.all[i].name + '</option>';
		}
		if(!found)
		{
	...
	*/
	blockParentInfo: function(bid, bs_id)
	{
		...
		if(item.blockfile.length) html += cmsEditorLang.selectTypeFile + ': ' + item.blockfile;
		else if(item['type'] == 1) html += cmsEditorLang.selectTypeBBcode;
		else html += cmsEditorLang.selectTypeHTML;
		// add button
	...
	*/
	addForm: function(key)
	{
		...
		if(!form.length)
		{
			var keys = {'active': cmsEditorLang.optionEnabled, 'border': cmsEditorLang.optionBorder, 'titlebar': cmsEditorLang.optionTitle, 'local': cmsEditorLang.optionLocal, 'background': cmsEditorLang.optionBackground},
