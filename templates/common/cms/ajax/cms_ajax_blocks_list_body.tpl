...
<tr>
	<td class="cat" colspan="9" style="text-align:left">
		...
		<div id="box-updated" class="row-center" style=" position: fixed; top: 0px; right: 25px; z-index: 1; background: none; border: none; width: 300px; padding: 3px;">
			<div id="result-box" style="height: 16px; border: solid 1px #228844; background: #00ff00;"><span class="text_green">{L_BLOCKS_POSITION_UPDATED}</span></div>
		</div>
		<!-- END blocks_updated -->
	</td>
</tr>
...
<tr>
	<td align="center">
		...
		<!-- BEGIN drop_blocks -->
			...
			{dropOnEmpty: true, handle: 'handle', containment: [{CONTAINMENT}], constraint: false, onChange: function(){$('list_{drop_blocks.BPOSITION}_debug').value = Sortable.serialize('list_{drop_blocks.BPOSITION}') }});
		<!-- END drop_blocks -->
		// ]]>
		</script>
		</div>
