...
{IMG_THL}{IMG_THC}<span class="forumlink">{L_EDIT_PIC_INFO}</span>{IMG_THR}<table class="forumlinenb">
<tr>
	<td class="row1 tw30pct"><span class="gen">{L_PIC_TITLE}</span></td>
	<td class="row2"><input class="post" type="text" name="pic_title" size="60" value="{PIC_TITLE}" /></td>
</tr>
<tr>
	<td class="row1 tw30pct"><span class="gen">{L_PIC_DESC}<br /></span><span class="genmed">{L_PLAIN_TEXT_ONLY}<br />{L_MAX_LENGTH}: <b>{S_PIC_DESC_MAX_LENGTH}</b></span></td>
	<td class="row2"><textarea class="post" cols="60" rows="4" name="pic_desc" style="width: 98%;">{PIC_DESC}</textarea></td>
</tr>
<tr>
	<td class="catBottom tdalignc" colspan="2"><input type="reset" value="{L_RESET}" class="liteoption" />&nbsp;&nbsp;&nbsp;<input type="submit" name="submit" value="{L_SUBMIT}" class="mainoption" /></td>
</tr>
