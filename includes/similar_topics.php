...
if (!defined('IN_ICYPHOENIX'))
{
	...
	for($i = 0; $i < $count_similar; $i++)
	{
		...
		$title = (strlen($similar['topic_title']) > 45) ? (substr($similar['topic_title'], 0, 42) . '...') : $similar['topic_title'];
		$topic_url = '<a href="' . append_sid(VIEWTOPIC_MG . '?' . POST_TOPIC_URL . '=' . $similar['topic_id']) . '" class="' . $topic_class . '">' . $title . '</a>';
		...
		$post_time = create_date2($board_config['default_dateformat'], $similar['topic_time'], $board_config['board_timezone']);
