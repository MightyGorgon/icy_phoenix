...
//<!-- END Unread Post Information to Database Mod -->
		...
		if ($user->data['session_logged_in'])
		{
			...
			$sql = "SELECT p.forum_id, p.topic_id, p.post_time
						FROM " . POSTS_TABLE . " p
						WHERE (p.post_time > " . $user_lastvisit . ") AND p.post_approval = " . POST_APPROVED . "
...
function make_cat_nav_tree($cur, $pgm = '', $meta_content = '', $nav_class = 'nav')
{
	...
	if (($type == POST_TOPIC_URL) || ($type == POST_POST_URL))
	{
		...
		elseif ($type == POST_POST_URL)
		{
			...
			$sql_where = " WHERE p.post_approval = " . POST_APPROVED . " AND t.topic_id = p.topic_id AND p.post_id = " . $id . " LIMIT 1";
		}
